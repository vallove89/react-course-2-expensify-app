...
test('shuld remove expense from firebase', (done) => {
...
test('should add expense to database to database.', (done) => {
  const store = createMockStore({});
  const expenseData = {
    description: 'Mouse',
    amount: 30000,
...
test('should add expense to database to database without data.', (done) => {
  const store = createMockStore({});
  const expenseData = {};
